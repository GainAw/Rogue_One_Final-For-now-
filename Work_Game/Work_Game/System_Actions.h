...
void Attack_Sequence() {
	...
	if (player.Health > 0) { player.Health = 100; delayed_out_2("\nVoice: 'I have restored your health'"); player.Monsters_Killed++; };
	if (player.Health <= 0) { delayed_out_2("\nVoice: 'You have been slain'"); Game_Run = false; };
...
void Wander_Halls() {
	...
	case 1: {
		switch (rand() % 3) {
		case 0: {left = true; top = true; }break;
		case 1: {right = true; top = true; }break;
	...
	switch (rand() % 3) {
	case 0:
	case 1: {player.In_Fight = true; }break;
	case 2: {player.In_Fight = false; }break;
	};
	player.Halls_Seen++;
};
...
void Mob_Fighter() {
	...
	case 4: {
		player.Mob_Name = skeleton.Name;
		player.Mob_Health = skeleton.Health;
		player.Mob_Damage = skeleton.Damage;
	}break;
	case 5: {
		player.Mob_Name = spirit.Name;
		player.Mob_Health = spirit.Health;
		player.Mob_Damage = spirit.Damage;
	}break;
	};
};
...
void Save_Game() {
	...
	save_file << player.Halls_Seen << endl;
	save_file << player.Monsters_Killed << endl;
...
void Load_Game() {
	...
	string Num_Holder;
	save_file.open("Save_File.txt");
	bool File_Empty = save_file.peek() == std::ifstream::traits_type::eof();
	if (File_Empty == true) { delayed_out_2("Please Don't tempt me\n"); }
	else {
		getline(save_file, player.Name);
		getline(save_file, Num_Holder);
		player.Age = stod(Num_Holder);
		getline(save_file, Num_Holder);
		player.Halls_Seen = stod(Num_Holder);
		getline(save_file, Num_Holder);
		player.Monsters_Killed = stod(Num_Holder);
		getline(save_file, Num_Holder);
...
void Tutorial() {
	...
	delayed_out_2("\nVoice: 'As for instance you can attack. Try it out' \n");
	while (true) {
		Ask_Action();
	...
	while (true) {
		...
		delayed_out_2("\nVoice: 'No Block it can be typed with caps or not!'/n");
	};
	delayed_out_2("\nYour health is "); cout << player.Health << endl;
